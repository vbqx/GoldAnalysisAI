...
### Phase A — 交易计划去误导化（当前实施）

| 任务 | 说明 | 验收 |
|------|------|------|
| A-1 信号状态 | 为每条 `TradingSignal` 增加 `status`: `candidate` / `watch` / `active` / `invalid` | UI 不再把所有候选区都显示成可执行计划 |
| A-2 触发确认 | 新增 `trigger_confirmed` 与 `trigger_note` | 未扫低收回、未反抽失败的信号必须显示「等待触发」 |
| A-3 质量评分 | 新增 `score_total` / `score_grade` / `score_reasons` | 每条计划展示结构、位置、R:R、触发缺口 |
| A-4 几何过滤 | SELL 区低于现价、BUY 未发生 sweep 时降级为候选 | 主交易计划不再把无触发区间当作 active |
...
输出应表达为：`背景方向 + 候选区 + 等待触发条件`，而不是单一买卖建议。
...
## 2026-06-21 Implementation Step: LLM-Generated Levels With Rule Validation

Phase A made trading plans less misleading by adding signal status, trigger notes and scoring. The next implemented slice adds LLM-generated candidate levels without removing the existing research team:

1. Analyst Team, Bullish Researcher, Bearish Researcher and Debate still produce the market thesis.
2. `compute_trading_signals(ctx)` still creates deterministic candidate zones.
3. `LLM_STAGE_LEVELS=true` lets the model propose entry/stop/target levels using the thesis and rule candidates.
4. `validate_llm_levels()` rejects invalid geometry and converts accepted proposals to `TradingSignal`.
5. Trader/Risk/Manager continue to consume the same signal contract.

This preserves the current LLM analysis framework while giving the LLM a controlled place to propose point levels.
