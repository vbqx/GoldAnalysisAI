...
按 `docs/aspice/governance/scope-and-closure.md` 的项目关闭准则，SWE.1–SWE.6
...
| 过程 | 结论 | 量化证据 | 关键工作产品 |
|---|---|---|---|
| SWE.1 | PASS | 26/26 发布软件需求属性完整；均有架构和验证链接 | [软件需求](../../../SWE.1-software-requirements.md)、[双向追溯](../../../traceability.md) |
| SWE.2 | PASS | 11 个组件、4 个受控架构接口、6 个集成步骤；双向链接无悬挂 | [软件架构](../../../SWE.2-architecture/software-architecture.md)、[IT](../../../SWE.5-integration-testing.md) |
| SWE.3 | PASS | 182/182 unit 和 1068/1068 function 有稳定 ID；逐函数设计字段 100% | [软件详细设计](../../../SWE.3-detailed-design/software-detailed-design.md) |
| SWE.4 | PASS | 182/182 unit 有选定措施；157 个高风险函数进入风险处置；阻断 unit 为 0 | [UT](../../../SWE.4-unit-testing.md)、417 项 unit 结果 |
| SWE.5 | PASS | 6/6 集成项含顺序、接口、前置、桩、超时、资源、用例和结果；离线 integration 2 项、回测 9 项通过 | [IT](../../../SWE.5-integration-testing.md)、结构化结果 |
| SWE.6 | PASS | 26/26 软件需求至少有一个接受结果；阻断需求为 0；26 项 regression 通过 | [VT](../../../SWE.6-validation-testing.md)、[验证结果](../../../records/verification/latest.md) |
...
393 个函数存在直接符号测试引用；其余函数依据风险选择静态验证和所属组件/集成验证。直接
...
结构化选择、环境、命令、结果和偏差见
`docs/aspice/records/verification/software-domain-2026-07-18.yaml`。
