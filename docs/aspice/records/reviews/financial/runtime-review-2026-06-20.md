...
主路径与 **45% bearish** 情绪权重一致（后续版本：`trend_projections` 会随主导情绪切换主路径模板）。  
**主图（2026-06 后）**：路径推演仅展示在底栏卡片，不在 5m 主图叠虚线；`test_chart_projections.py` 覆盖投影轴与主图默认行为。  
...
完整规划见 **[static-code-review.md §7](./static-code-review.md#7-修复路径规划2026-06-20)**。
