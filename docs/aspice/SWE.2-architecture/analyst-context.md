...
| **Layer 2 — Derived** | `data/context_builder.py` | EMA/VWAP 位置、结构情绪、**upcoming** 日历高影响计数 |
| **Layer 3 — Evidence** | `agents/analysts/*` | `AnalystReport.items`（规则 + LLM 共用） |

日历过滤与 legacy 文本同步：

- `data/calendar_utils.py` — 解析/过滤 **未来** 日历（无法解析时间的事件丢弃，不默认视为即将发生）
- `data/external_format.py` — `sync_external_legacy_fields()` 仅将 **upcoming** 日历写入 `risk_events`
- `finalize_market_context()` 会就地替换 `ExternalFactors.calendar_events` 为过滤后列表
...
`ExternalFactors` 同时保留 legacy 字段 `news_headlines` / `risk_events`（供 UI 与旧逻辑），并由 `sync_external_legacy_fields()` 从 **已过滤的 upcoming 日历** 自动同步。
...
以上能力已在规则技术分析师、LLM 技术 payload 与最终 narrative payload 中落地；指标权重和历史校准计划见 [roadmap.md](../../management/roadmap.md)。
