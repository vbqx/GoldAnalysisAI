...
```mermaid
flowchart TB
    TV["TradingView 行情"] --> FACT["enrich + ict_pa.analyze<br/>确定性规则事实"]
    FACT --> TEAM["Analyst Team<br/>技术 / 基本面 / 新闻 / 情绪"]
    MODE["agents/factory.py<br/>rule / llm / hybrid"] --> TEAM
    TEAM --> BULL["看多研究<br/>research_payload"]
    TEAM --> BEAR["看空研究<br/>research_payload"]
    BULL --> DEBATE["辩论与反证<br/>debate_payload"]
    BEAR --> DEBATE
    DEBATE --> TRADER["交易员<br/>读取辩论结论 + 候选信号"]
    TRADER --> RISK["风险团队<br/>三档审查"]
    RISK --> MANAGER["经理<br/>只读提案 + 风控结果"]
    MANAGER --> REPORT["report_engine.build_report<br/>信号角色与报告组装"]
    REPORT --> NARR["llm/analyst.py<br/>可选报告文案层"]
    NARR --> UI["LLM 决策链页面<br/>来源标识 + 完整 I/O 历史"]
```
...
```mermaid
flowchart LR
    INPUT["MarketContext<br/>+ 紧凑事实目录"] --> SYN["单次 LLM 决策综合"]
    SYN --> ADAPTER["领域适配器<br/>AnalystTeam / ResearchDebate<br/>LevelProposal / TransactionProposal"]
    ADAPTER --> GATES["现有确定性门禁<br/>价位 / 主张 / 触发 / 时效 / 风险"]
    GATES --> MANAGER["Manager 最终授权"]
    MANAGER --> NARR["可选叙事 LLM<br/>不得反向改变授权"]
```
...
| 文档 | 内容 |
|------|------|
| [software-domain.md](../software-domain.md) | ASPICE 软件域索引 |
| [architecture.md](./system-overview.md) | TradingAgents 对照与分层图 |
| [analyst-context.md](./analyst-context.md) | Analyst 输入密度与 payload |
| [roadmap.md](../../management/roadmap.md) | 后续执行接口、MT5 接入和 ICT 标准化计划 |
...
研究层仍是 Analyst Team -> Bullish/Bearish Research -> Debate。LLM 点位阶段插入 Debate 与 Trader 之间；完整架构说明见 [architecture.md §8.1](./system-overview.md#81-llm-点位层)。
...
Trader / Risk / Manager 三段 LLM 已接入 `agents/factory.py` 双轨调度；后续实盘执行与 MT5 接入计划见 [roadmap.md](../../management/roadmap.md)。
