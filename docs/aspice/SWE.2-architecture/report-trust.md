...
文档索引：[architecture.md](./system-overview.md) · [llm-agents.md](./llm-agents.md) · [analyst-context.md](./analyst-context.md) · [report-schema.md](../SWE.3-detailed-design/reference/examples/report-schema.md)
...
| `{tf}.*` | `4h.swing_high`, `4h.ob.0.low`；FVG 另含 `width` / `width_atr_ratio` / `atr`（主张资格） | Lux SMC |
...
| 代码 | 检查项 |
|------|--------|
| `INV-GEO-*` | **仅**已授权/`primary`/`alternate` 方案的 entry/SL/TP 几何与相对现价位置（候选库存不算交付一致性失败） |
| `INV-AUTH-*` | wait/observation 下叙事含可执行措辞（扩展中英文黑名单+正则） |
| `INV-MGR-*` | 经理 wait vs 结论标题矛盾 |
| `INV-PRICE-*` | LLM 叙事中未注册价位（**小数仍用 0.51**；仅无小数点的整数心理关口放宽至 5） |
| `INV-FRESH-*` | 陈旧快照 + 急迫执行措辞 |
| `INV-META-*` | 缺 narrative 审计元数据 |
| `INV-FACT-*` | fact_registry 冲突 |
| `INV-TRIG-001` | `execution_authorized=true` 但主计划 `trigger_confirmed=false`（#35） |
| `INV-CLAIM-001` | `execution_authorized=true` 但核心主张非 `core_execution`（#36） |

未通过时执行 **`apply_report_invariant_gate`**：对授权类违规撤销执行授权、清空违规 LLM 字段、重写结论、`pipeline_status=degraded`；`meta.warnings` 仍追加摘要供 UI 展示。

---

## 5.1 计划获准 vs 执行就绪（#35）

`apply_manager_authorization` 区分两层：

| 字段 | 含义 |
|------|------|
| `meta.plan_authorized` | Manager 批准保留该条件计划（UI 可展示入场区） |
| `meta.execution_ready` / `execution_authorized` | 触发已确认 **且** 数据可执行 **且** scale>0 **且** 主张资格允许 |
| `meta.plan_signal_ids` | 获准计划 ID（含等待触发） |
| `meta.authorized_signal_ids` | 仅执行就绪时非空 |
| `meta.primary_trigger_state` | 主计划触发/资格快照（含 `claim_eligibility`） |

未触发时仓位显示为「0% 等待触发」；Risk 可保留索引但 `position_scale=0`。不得用「缩仓执行」掩盖条件状态。

---

## 5.2 技术主张引用资格（#36，`claim-v2`）

**模块**：`analysis/claim_eligibility.py` → Levels 校验与 Manager 授权共用。

| 资格 | 可否进入执行授权 |
|------|------------------|
| `core_execution` | 可以（须同时满足 #35 触发门禁） |
| `supporting` / `observation_only` | 否；可作条件计划/观察背景 |
| `uncitable` | 否；`level_validator` 拒绝该 proposal |
| 空（规则/旧信号） | 兼容放行，不强制要求 claim 字段 |

裁决写入 `LevelProposal` / `TradingSignal`：`claim_id`、`claim_fact_ids`、`claim_quality`、`counterevidence`、`claim_eligibility`、`claim_policy_version`。

`claim-v2` 在 v1 的 FVG 宽度/反向重叠门禁上增加结构化关系验证：Technical reaction 必须从 `technical_claim_facts` 引用完整 `fact_ids`，并声明 `overlap` / `near` / `contradiction` 关系。门禁会复算 FVG、OB、PA S/R、VA/POC 的几何关系及其与入场区的连通性；未知、部分引用、自由文本共振、关系不成立或与入场区断开的事实均不得成为 `core_execution`。有效/无效关系轨迹写入 `claim_quality.relationship_audit` 与 `counterevidence`。阈值可后续用点时回测表替换，须带策略版本。

流水线插入点：`validate_llm_levels`（注入 `level_reactions`）→ `apply_manager_authorization`（`claim_ok`）→ `INV-CLAIM-001`。
...
pytest tests/unit/test_risk_gates_trigger.py
...
| 调整主张资格 / ATR 门槛 | `analysis/claim_eligibility.py` |
| 计划 vs 执行授权 | `analysis/report_engine.py` (`apply_manager_authorization`) |
...
更完整的文件→测试对照见 [cheat-sheet.md](../SWE.3-detailed-design/reference/cheat-sheet.md)。
