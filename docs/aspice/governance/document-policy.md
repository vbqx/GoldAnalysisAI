...
| 层级 | 目录 | 权威性 | 是否人工编辑 |
|---|---|---|---|
| 当前设计 | `docs/aspice/` 的 SWE.1–SWE.6、SUP.8、治理文档 | 当前评审基线 | 治理与专题人工编辑；过程主文档由结构化模型生成 |
| 结构化自动化 | `docs/aspice/_machine/` | 需求、架构、验证与配置模型，以及源码扫描证据 | YAML 按变更评审修改；CSV/JSON/锁文件自动生成 |
...
1. SWE.1、SWE.2 主架构、SWE.4、SWE.5、SWE.6、SUP.8 与追溯矩阵由 `_machine/` 中经过评审的 YAML/CSV 模型生成；Markdown 是普通评审入口，不直接手改生成段落。
2. SWE.3 组件函数卡片由源码 AST 和验证映射生成；人工补充内容写入 `critical-units.md` 或 reference 文档。
3. `_machine/` 中 YAML 是 ID、关系和配置属性的受控源；CSV、JSON、锁文件和过程索引是自动生成证据。任何 YAML 变更必须同时通过可读文档和追溯检查。
...
- 禁止目录没有重新出现；
- 业务代码差异与已批准变更范围一致，并已反映到需求、架构、详细设计和验证证据。
