...
软件单元边界以模块语义为主，不等同于每个 Python 函数。全部模块与函数设计从 [SWE.3 组件导航](./README.md)进入；低风险 helper 继承所属组件设计，以下保留高风险单元的人工专项说明。
...
未在本文件单列的模块，其静态接口由 [SWE.3 软件详细设计](./README.md) 的函数/类清单和类型注解定义，动态职责由 [SWE.2 软件架构](../SWE.2-architecture/README.md) 定义。若模块包含以下任一特征，必须新增专门章节：执行授权、外部副作用、归档写入、并发、超过 200 行的公开函数、关键算法或安全边界。
