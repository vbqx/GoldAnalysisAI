...
| `llm_io` | 规则阶段输入输出 + LLM 消息与响应；LLM 行含 `input_chars`、token 估算、`attempt(s)`、`budget_action`、`tier`、`usage`（供应商 token，未返回则为 `null`） |
| `llm_routing` | `llm-stage-v1` 策略快照：`same_model_strategy`、FAST/STRONG/REPORT、每阶段预算 |
| `audit_summary.llm_usage_summary` | 每轮聚合：估算字符/token、重试原因、预算动作、`provider_usage_available` 与 `provider_*_tokens` |
| `context_stats` | 分析师输入密度，见 [analyst-context.md](../../../SWE.2-architecture/analyst-context.md) |
...
来自 `ict_pa.sentiment_score()` 多周期趋势加权。界面若展示为「胜率」，须标注为**结构权重**。见 [financial-review.md](../../../records/reviews/financial/static-code-review.md) F-002。
...
**执行语义**：`trigger_confirmed=false` 或 `claim_eligibility` 非 `core_execution`（且非空）时，不得将计划呈现为执行就绪；见 [report-trust.md](../../../SWE.2-architecture/report-trust.md) §5.1–5.2。
...
Research 侧 `bullish` / `bearish` 条目含 `provenance_meta`（上游覆盖率、来源多样性、确定性置信分量）及带 `evidence_id` + `refs` 的 `items`。详见 [report-trust.md](../../../SWE.2-architecture/report-trust.md)。
...
完整规则见 [report-trust.md](../../../SWE.2-architecture/report-trust.md)。
...
sample = json.loads(Path("docs/aspice/SWE.3-detailed-design/reference/examples/sample-report.json").read_text(encoding="utf-8"))
print(sample.keys())
