...
MT5 不在这条行情/回测链路中。MT5 只作为账号检查和后续 `shadow` / `paper_mt5` / `live_mt5` 执行通道，边界见 [architecture/review.md](../../SWE.2-architecture/health-review.md)。
...
| `LLM_TIMEOUT`, `LLM_CONNECT_TIMEOUT`, `LLM_READ_TIMEOUT` | 连接与流式空闲超时（秒）；见 [llm-agents.md §3.4](../../SWE.2-architecture/llm-agents.md#34-传输重试预算与遥测) |
...
3. 改测试分层或输出边界：同步 [testing/strategy.md](../../governance/verification-strategy.md) 与 `tests/cases/catalog.yaml`。
4. 改架构边界：同步 [architecture/review.md](../../SWE.2-architecture/health-review.md)，不要把当前事实只写在 roadmap。
...
| [llm-agents.md](../../SWE.2-architecture/llm-agents.md) | LLM 智能体设计 |
| [report-trust.md](../../SWE.2-architecture/report-trust.md) | 事实注册表、证据溯源、不变量、可靠度 |
| [backtesting.md](../../SWE.2-architecture/backtesting.md) | 回测边界 |
| [strategy.md](../../governance/verification-strategy.md) | 测试策略 |
