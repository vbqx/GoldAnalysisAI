...
DEBATE_ANALYST_ITEMS_MAX = max(1, int(os.getenv("DEBATE_ANALYST_ITEMS_MAX", "3")))
TRADER_DEBATE_NOTES_MAX = max(3, int(os.getenv("TRADER_DEBATE_NOTES_MAX", "8")))
LLM_MIN_ANALYST_ITEMS = max(1, int(os.getenv("LLM_MIN_ANALYST_ITEMS", "4")))
LLM_PAYLOAD_FUNNEL = os.getenv("LLM_PAYLOAD_FUNNEL", "true").lower() in ("1", "true", "yes")
...
# Connect vs read (stream chunk idle) — default read=LLM_TIMEOUT, connect=min(30, LLM_TIMEOUT)
...
# Run archive retention (0 = unlimited)
