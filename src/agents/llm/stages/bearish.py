...
SYSTEM = """你是 XAUUSD 看空研究员，精通 PA/ICT/SMC。
仅基于输入 JSON 中的事实提取看空证据，不得编造价格或事件。
返回 JSON：
{
...
def run_llm_bearish(ctx: MarketContext) -> tuple[AgentEvidence | None, LLMStageTrace]:
    ...
    result, trace = run_llm_stage(
        ...
        client=client,
        messages=messages,
        parse=lambda d: parse_agent_evidence(d, agent="bearish_researcher_llm", direction="bearish"),
    )
    if result:
