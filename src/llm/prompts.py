...
from src.analysis.field_glossary import (
...
SYSTEM_PROMPT = f"""你是专业的 XAUUSD 黄金日内交易分析师，精通 LuxAlgo SMC 与 PA 量价分析。
{INTRADAY_GOLD_MANDATE}
...
2. 用中文输出，风格简洁专业，面向日内交易员
3. trade_thesis 先写 4H/1H 方向；action_plan / watch_levels 用 15m/5m PA 定区（POC/VAH/VAL、量价 S/R）；SMC 不进计划文案
...
{{
  ...
  "action_plan": "观察模式下留空；仅授权执行时写 2-4 条可执行步骤（含入场/止损）",
  "risks": ["风险1", "风险2", "风险3"],
...
每块总计3至6行：当前状态、关键区间、条件分支、失效条件。context / levels / conditions 必须是字符串数组（即使只有一句也要用 ["..."]，禁止写成单个字符串）。context 最多保留 1 条、levels 最多 2 条、conditions 最多 1 条（超出部分会自动截断，不会判错）。
...
def _observation_hint(context: dict[str, Any]) -> str:
    ...
    return (
        ...
        "action_plan 必须留空；仅使用 watch_levels 列出 2-4 个关键观察价位（不含入场/止损/止盈几何）。"
        "禁止任何可执行措辞（如追空/追多/可考虑做空/做多/等待反弹做空/立即入场等）。"
    )


def build_messages(context: dict[str, Any]) -> list[dict[str, str]]:
    user_content = (
        "请根据以下 XAUUSD 黄金日内分析数据，生成机构级文字结论（JSON；高周期定方向，15m/5m 定执行）："
