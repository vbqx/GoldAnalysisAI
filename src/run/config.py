...
class RunConfig:
    ...
    def normalized(self) -> "RunConfig":
        replay_run_id = (self.replay_run_id or "").strip()
    ...
    def fingerprint(self) -> str:
        cfg = self.normalized()
        if cfg.replay_mode and cfg.replay_run_id:
            raw = json.dumps({"replay_mode": True, "replay_run_id": cfg.replay_run_id}, ensure_ascii=False, sort_keys=True)
            return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
        raw = json.dumps(cfg.to_dict(), ensure_ascii=False, sort_keys=True)
        return hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_config_for_mode(mode: AgentMode, *, llm_enabled: bool = True, llm_analyst_only: str = "") -> RunConfig:
    ...
    return RunConfig(
        ...
        llm_stage_trader=mode == "llm",
        llm_stage_risk=mode == "llm",
        llm_stage_manager=mode == "llm",
