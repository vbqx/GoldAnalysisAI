...
def create_issue(title: str, body: str, labels: list[str]) -> str:
    r = api("POST", f"/repos/{OWNER}/{REPO}/issues", {"title": title, "body": body, "labels": labels})
    print(f"Created #{r['number']}: {r['html_url']}")
    return r["html_url"]


ISSUES = [
    {
        "title": "[Testing] 系统测试报告 2026-06-14 — GoldAnalysisAI 全功能测试",
        "labels": ["testing"],
        "body": """## 测试概要
...
""",
    ...
    {
        "title": "[Bug] LLM 阶段 JSON 解析偶发失败，hybrid 模式回退规则引擎",
        "labels": ["bug", "testing", "priority: high"],
        "body": """## 测试场景
...
""",
    ...
    {
        "title": "[Improvement] run_pipeline_test.py 直接运行报 ModuleNotFoundError",
...
""",
    ...
    {
        "title": "[Improvement] Streamlit use_container_width 弃用警告",
        "labels": ["enhancement", "testing", "priority: low"],
        "body": """## 测试场景
...
""",
    ...
    {
        "title": "[Improvement] 侧边栏仅显示 STRONG 模型，与研究阶段 FAST 模型不一致",
        "labels": ["enhancement", "testing", "priority: low"],
        "body": """## 测试场景
...
""",
    ...
    {
        "title": "[Improvement] 子页面首次加载缺少流水线进度 UI",
        "labels": ["enhancement", "testing", "priority: low"],
        "body": """## 测试场景
...
def main() -> None:
    for ld in [
        ...
        ("testing", "0075ca", "Testing related"),
