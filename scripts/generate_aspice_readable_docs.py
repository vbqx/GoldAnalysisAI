...
def expected_outputs() -> dict[Path, str]:
    ...
    outputs[ASPICE / "SWE.1-software-requirements.md"] = _requirements_doc(reqs)
    outputs[ASPICE / "SWE.2-architecture" / "software-architecture.md"] = _architecture_doc(arch, units)
    outputs[ASPICE / "SWE.4-unit-testing.md"] = _unit_verification_doc(arch, unit_rows)
    outputs[ASPICE / "SWE.5-integration-testing.md"] = _integration_doc(integration, measures)
    outputs[ASPICE / "SWE.6-validation-testing.md"] = _qualification_doc(measures, coverage)
    outputs[ASPICE / "SUP.8-configuration-management.md"] = _configuration_doc(cm)
    outputs[ASPICE / "traceability.md"] = _traceability_doc(reqs, coverage)
    return outputs
